...
export const PostNew = () => {
  ...
  const onSubmit = async () => {
    if (prompt) {
      try {
        // Postテーブルにデータを追加
        const { data: insertData, error: insertError } = await supabase.from("posts").insert({ prompt }).select()
  ...
  return (
    <div className="fixed bottom-0 left-2 right-2 h-40 flex flex-col justify-end items-center bg-[#7494C0] pb-5">
      ...
      <textarea
        className="w-[752px] bg-gray-50 rounded py-3 px-3 outline-none focus:bg-white text-gray-900"
        id="prompt"
        name="prompt"
